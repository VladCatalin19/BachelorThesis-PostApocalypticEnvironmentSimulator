...
namespace Ivy
{
    ...
    public class IvyCore
    {
        ...
        static double refreshInterval = 0.9;
        ...
        private static IvyDataAsset dataAsset = null;
        ...
        public static void Update(IvyBehavior ivyBheavior, float probability)
        {
            CacheTerrainColliderStuff();
            adhesionMeshCache.Clear();
            ...
            if (ivyBheavior == null || ivyBheavior.profileAsset == null)
            {
                return;
            }
            int graphsGrowing = 0;
            foreach (var ivy in ivyBheavior.ivyGraphs)
            {
                if (Random.Range(0.0f, 1.0f) < probability)
                {
                    if (ivy.isGrowing)
                    {
                        GrowIvyStep(ivy, ivyBheavior.profileAsset.ivyProfile);

                        if (ivy.generateMeshDuringGrowth)
                        {
                            IvyMesh.GenerateMesh(ivy, ivyBheavior.profileAsset.ivyProfile);
                        }
                        grewThisFrame = true;
                        needToSaveAssets = true;
                        graphsGrowing++;
                    }
                    if (!ivy.isGrowing && ivy.generateMeshDuringGrowth && ivyBheavior.profileAsset.ivyProfile.useLightmapping && ivy.dirtyUV2s)
                    {
                        IvyMesh.GenerateMesh(ivy, ivyBheavior.profileAsset.ivyProfile, true);
                        grewThisFrame = true;
                        needToSaveAssets = true;
                    }
                }
        ...
        public static void StopGrowing(IvyBehavior ivyBheavior)
        {
            foreach (var ivy in ivyBheavior.ivyGraphs)
            {
                ivy.isGrowing = false;
        ...
        public static void StartGrowing(IvyBehavior ivyBheavior)
        {
            foreach (var ivy in ivyBheavior.ivyGraphs)
            {
                ivy.isGrowing = true;
        ...
        public static void OpenExampleScene () {
            ...
            if ( scenes != null && scenes.Length > 0) {
                ...
                UnityEngine.Debug.LogWarning( "Hedera > Open HederaExampleScene couldn't find the example scene, which is usually in Assets/Hedera/Example/HederaExampleScene.unity ... you may have renamed the scene file or deleted it.");
                return;
            }
